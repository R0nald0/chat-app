...
class AuthRepositoryImpl implements AuthRepository {
  ...
  AuthRepositoryImpl({
    required ChatAuthRestClient chatRestClient,
    required FlutterSecureStorage storage,
  }) : _restClient = chatRestClient,
       _storage = storage;
  ...
  Future<User> login(({String email, String password}) data) async {
    try {
      ...
      final ChatResponseDto(:user, :token) = await _restClient.unAuth().login(
        data,
      );
      ...
      if (!isSaved) {
        ...
        throw RepositoryException(
          message: 'Erro ao salvar dados do usuário realizar o login novamente',
        );
      }
  ...
  Future<User?> logout() async {
    try {
      ...
      final isClearData = await pref.clear();
      if (!isClearData) {
        throw RepositoryException(message: 'Falha ao deslogar usuário');
      }
      return null;
    } on RepositoryException catch (e,s) {
       log('Falha ao deslogar usuário' ,error : e, stackTrace: s);
